...
export default async function CoursePage({ params }: { params: { slug: string } }) {
  ...
  return (
    <div className="mx-auto max-w-6xl p-6">
      {/* Hero */}
      <div className="relative overflow-hidden rounded-4xl bg-gradient-to-br from-indigo-600 via-indigo-500 to-violet-600 p-6 text-white shadow-[0_18px_56px_rgba(49,46,129,0.35)] ring-1 ring-indigo-900/20">
        {/* Decorative blobs */}
        <div className="pointer-events-none absolute -left-20 -top-20 h-56 w-56 rounded-full bg-white/10 blur-3xl" />
        <div className="pointer-events-none absolute -right-16 -bottom-16 h-64 w-64 rounded-full bg-fuchsia-300/10 blur-3xl" />
        <div className="relative flex items-start justify-between gap-4">
          <div>
            <span className="inline-flex rounded-full bg-white/15 px-3 py-1 text-[10px] font-bold uppercase tracking-wide">Course</span>
            <h1 className="mt-2 text-3xl font-extrabold leading-tight tracking-tight sm:text-4xl">{course.title}</h1>
            {course.description ? (
              <p className="mt-1 max-w-2xl text-sm text-white/90">{course.description}</p>
            ) : null}
          </div>
        </div>
      </div>
