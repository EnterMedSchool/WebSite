...
async function loadPending() {
  const rows = (await sql`SELECT r.id, r.user_id, r.university_id, r.note, r.created_at, u.name, u.username, u.email
...
export default async function UniversityModeratorRequestsPage() {
  ...
  return (
    <div className="mx-auto max-w-5xl px-4 py-8">
      <h1 className="text-2xl font-semibold">University Moderator Applications</h1>
        ...
        {rows.map((r)=> (
          <div key={r.id} className="grid grid-cols-[1fr,1fr,auto] items-center gap-4 p-4">
            ...
            <div className="text-sm text-gray-800">
              <div>University #{r.university_id}</div>
              <div className="text-xs text-gray-600">{new Date(r.created_at).toLocaleString()}</div>
            </div>
            <div className="flex gap-2">
              <form action={`/api/admin/universities/moderators/requests/${r.id}/approve`} method="post"><button className="rounded bg-emerald-600 px-3 py-1.5 text-xs font-semibold text-white">Approve</button></form>
              <form action={`/api/admin/universities/moderators/requests/${r.id}/reject`} method="post"><button className="rounded bg-rose-600 px-3 py-1.5 text-xs font-semibold text-white">Reject</button></form>
            </div>
          </div>
        ))}
