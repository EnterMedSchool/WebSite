-- Italy IMAT 2021 real data seed
-- Usage: psql <conn args> -f docs/db/italy_imat_2021_seed.sql
...
CREATE TEMP TABLE tmp_imat2021 (
  name text,
  city text,
...
INSERT INTO tmp_imat2021 (name, city, lat, lng, kind, language, exam, program, eu_seats, eu_last, noneu_seats, noneu_last)
VALUES
  ...
  ('Bicocca', 'Milan', 45.5196, 9.2139, 'public', 'English', 'IMAT', 'Medicine and Surgery', 22, 36.3, 13, 42.3),
  ...
  ('Bari', 'Bari', 41.1171, 16.8719, 'public', 'English', 'IMAT', 'Medicine and Surgery', 42, 33.9, 8, 32.7);
...
-- Map names to IDs
CREATE TEMP TABLE tmp_imat2021_ids AS
...
JOIN tmp_imat2021 t ON t.name = u.name;
...
FROM tmp_imat2021_ids i
LEFT JOIN university_programs p ON p.university_id = i.id AND p.name = i.program
WHERE p.id IS NULL;

-- 2021 scores and seats (avoid duplicates).
-- Scores: EU
INSERT INTO university_scores (university_id, year, candidate_type, min_score)
SELECT i.id, 2021, 'EU', i.eu_last
FROM tmp_imat2021_ids i
LEFT JOIN university_scores s ON s.university_id = i.id AND s.year = 2021 AND s.candidate_type = 'EU'
WHERE s.id IS NULL;

-- Scores: NonEU
INSERT INTO university_scores (university_id, year, candidate_type, min_score)
SELECT i.id, 2021, 'NonEU', i.noneu_last
FROM tmp_imat2021_ids i
LEFT JOIN university_scores s ON s.university_id = i.id AND s.year = 2021 AND s.candidate_type = 'NonEU'
WHERE s.id IS NULL;

-- Seats: EU
INSERT INTO university_seats (university_id, year, candidate_type, seats)
SELECT i.id, 2021, 'EU', i.eu_seats
FROM tmp_imat2021_ids i
LEFT JOIN university_seats s ON s.university_id = i.id AND s.year = 2021 AND s.candidate_type = 'EU'
WHERE s.id IS NULL;

-- Seats: NonEU
INSERT INTO university_seats (university_id, year, candidate_type, seats)
SELECT i.id, 2021, 'NonEU', i.noneu_seats
FROM tmp_imat2021_ids i
LEFT JOIN university_seats s ON s.university_id = i.id AND s.year = 2021 AND s.candidate_type = 'NonEU'
WHERE s.id IS NULL;
