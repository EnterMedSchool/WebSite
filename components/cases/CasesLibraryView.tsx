...
export default function CasesLibraryView() {
  ...
  const [subjectFilter, setSubjectFilter] = useState<string>(state.activeSubjectSlug ?? "All");

  useEffect(() => {
    if (subjectFilter !== "All") {
      setSubjectFilter(state.activeSubjectSlug);
    }
  }, [state.activeSubjectSlug, subjectFilter]);
  ...
  const filtered = bundle.cases.filter((c) => {
    ...
    const matchesSubject = subjectFilter === "All" || c.subjectSlug === subjectFilter;
    return matchesQuery && matchesSystem && matchesDiff && matchesSubject;
  });
        ...
        <div className="grid grid-cols-3 gap-3 text-sm text-slate-200">
          ...
          >
            {subjectOptions.map((slug) => (
              <option key={slug} value={slug} className="text-slate-900">
                {slug === "All" ? "All subjects" : bundle.subjects.find((subject) => subject.slug === slug)?.name ?? slug}
              </option>
            ))}
          </select>
          <select
