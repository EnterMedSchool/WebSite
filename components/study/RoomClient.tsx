...
export default function RoomClient({ room, messages, taskLists, myUserId }: { room: any; messages: any[]; taskLists: any[]; myUserId: number | null; }) {
  ...
  useEffect(() => {
    ...
    const loadTasks = async () => {
      ...
      if (res.ok) {
        const json = await res.json();
        const lists = (json.data || []).map((l: any) => ({ id: l.id, title: l.title, userId: l.userId, updatedAt: l.updatedAt ?? null, items: (l.items || []).map((it: any) => ({ id: it.id, name: it.name, isCompleted: it.isCompleted, xpAwarded: it.xpAwarded })) }));
