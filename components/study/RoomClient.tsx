...
export default function RoomClient({ room, messages, taskLists, myUserId }: { room: any; messages: any[]; taskLists: any[]; myUserId: number | null; }) {
  ...
  useEffect(() => {
    ...
    setTaskLists((taskLists || []).map((l: any) => ({ id: l.id, title: l.title, userId: l.userId, items: (l.items || []).map((it: any) => ({ id: it.id, name: it.name, isCompleted: it.isCompleted, xpAwarded: it.xpAwarded })) })));
  ...
  useEffect(() => {
    if (!sessionId) return;
    const join = async () => { await fetch(`/api/study/sessions/${encodeURIComponent(room.slug)}/join`, { method: "PATCH", credentials: "include" }); };
    const leave = async () => { await fetch(`/api/study/sessions/${encodeURIComponent(room.slug)}/leave`, { method: "PATCH", credentials: "include" }); };
    ...
    const loadTasks = async () => {
      ...
      if (res.ok) {
        const json = await res.json();
        const lists = (json.data || []).map((l: any) => ({ id: l.id, title: l.title, userId: l.userId, items: (l.items || []).map((it: any) => ({ id: it.id, name: it.name, isCompleted: it.isCompleted, xpAwarded: it.xpAwarded })) }));
