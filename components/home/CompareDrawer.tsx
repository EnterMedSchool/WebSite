...
function insights(items: Item[]): string {
  ...
  const parts: string[] = [];
  if (bestRating) parts.push(`${bestRating.uni} has the highest rating${bestRating.rating ? ` (${bestRating.rating.toFixed(1)})` : ''}.`);
  if (topScore) parts.push(`${topScore.uni} shows the highest last admission score${topScore.lastScore ? ` (${topScore.lastScore}/100)` : ''}.`);
  if (langs.length) parts.push(`Languages across your picks: ${langs.join(', ')}.`);
  if (exams.length) parts.push(`Admissions covered: ${exams.join(', ')}.`);
  return parts.join(' ');
}
...
function buildPaths(series: Series[], cand: 'EU'|'NonEU'|'All') {
  const colors = ["#6C63FF", "#F59E0B", "#10B981", "#EF4444", "#3B82F6", "#A855F7", "#14B8A6", "#6366F1", "#F97316", "#8B5CF6"];
  const years = Array.from(new Set(series.flatMap(s=> s.points.map(p=>p.year)))).sort((a,b)=>a-b);
  const minScore = Math.min(0, ...series.flatMap(s=> s.points.map(p=> Number(p.score)||0)));
  const maxScore = Math.max(100, ...series.flatMap(s=> s.points.map(p=> Number(p.score)||0)));
  ...
  const y = (score: number) => {
    const t = (Number(score) - minScore) / (maxScore - minScore || 1);
    return H - P - t * (H - 2*P);
  };
  const paths = series.map((s, i) => {
    const pts = years.map((yr)=> {
      const m = s.points.find(p=> p.year===yr && (cand==='All' ? true : p.type===cand));
      return m ? { x: x(yr), y: y(m.score) } : null;
    }).filter(Boolean) as Array<{x:number;y:number}>;
    const d = pts.reduce((acc,p,idx)=> acc + (idx===0?`M ${p.x},${p.y}`:` L ${p.x},${p.y}`), '');
    return { d, color: colors[i%colors.length], uni: s.uni };
  });
  return { paths, years };
}
