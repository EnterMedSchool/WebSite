...
function buildPaths(
  series: Series[],
  cand: "EU" | "NonEU" | "All"
): { paths: Array<{ d: string; color: string; uni: string }>; years: number[] } {
  const colors = [
    "#6C63FF",
    "#F59E0B",
    "#10B981",
  ...
  const paths = series.map((s, i) => {
    const pts = years
      .map((yr) => {
        const m = s.points.find((p) => p.year === yr && (cand === "All" ? p.type === "All" || p.type == null : p.type === cand));
                      ...
                      <>
                        ...
                        <svg width="100%" viewBox="0 0 640 220" className="rounded-lg bg-gray-50">
                          ...
                          {years.map((yr, i) => (
                            <text
                              key={yr}
                              x={30 + (i / Math.max(1, years.length - 1)) * (610 - 30)}
                              y="210"
                              fontSize="10"
                              textAnchor="middle"
                              fill="#64748B"
