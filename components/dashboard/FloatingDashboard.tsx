                    ...
                    <div key={ch.id} className="flex flex-col justify-between rounded-2xl border border-gray-100 p-4 shadow-[0_10px_22px_rgba(0,0,0,0.06)]">
                      <div>
                        ...
                        <div className="mt-2 flex items-center gap-3 text-xs text-gray-600">
                          <span className="inline-flex items-center gap-1 rounded-full bg-emerald-50 px-2 py-0.5 font-semibold text-emerald-700 shadow-sm">79% done</span>
                          <span className="opacity-60">-</span>
                          <span>45 min</span>
                        </div>
                      </div>
                      <div className="mt-3 flex items-center gap-2">
                        <a href={`/course/${encodeURIComponent(ch.course_slug)}`} className="rounded-full bg-gradient-to-r from-indigo-600 to-fuchsia-600 px-3 py-1.5 text-sm font-semibold text-white shadow hover:opacity-95">Continue</a>
...
type DashData = {
  ...
  learning: { minutesToday: number; minutesTotal: number; correctToday: number };
  chapters: { id: number; slug: string; title: string; course_slug: string; course_title: string; total_min?: number | null; progress_pct?: number | null; continue_slug?: string | null }[];
  courses: { id: number; slug: string; title: string; description?: string | null }[];
};
                ...
                <div className="mt-3 grid gap-3 md:grid-cols-2">
                  {(data?.chapters?.length ? data.chapters : []).slice(0, 2).map((ch) => (
                    <div key={ch.id} className="flex flex-col justify-between rounded-2xl border border-gray-100 p-4 shadow-[0_10px_22px_rgba(0,0,0,0.06)]">
                      ...
                      <div className="mt-3 flex items-center gap-2">
                        <a href={ch.continue_slug ? `/lesson/${encodeURIComponent(ch.continue_slug)}` : `/course/${encodeURIComponent(ch.course_slug)}`} className="rounded-full bg-gradient-to-r from-indigo-600 to-fuchsia-600 px-3 py-1.5 text-sm font-semibold text-white shadow hover:opacity-95">Continue</a>
                        <a href={`/courses`} className="rounded-full border border-gray-300 px-3 py-1.5 text-sm font-semibold text-gray-700 hover:bg-gray-50">Skip</a>
                      </div>
                    </div>
                  ))}
              ...
              <div className="overflow-hidden rounded-3xl border border-gray-100 bg-white/90 shadow-[0_10px_30px_rgba(99,102,241,0.10)]">
                ...
                <div className="p-4">
                  ...
                  <div className="mt-3 grid grid-cols-3 gap-2 text-center text-xs">
                    ...
                    <div className="rounded-xl border p-2">
                      <div className="text-lg font-extrabold text-gray-900">{data?.user?.streakDays ?? '—'}</div>
                      <div className="text-gray-600">Streak</div>
                    </div>
                  </div>
                </div>
