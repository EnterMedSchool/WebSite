...
export default function FloatingDashboard({ open, onClose }: { open: boolean; onClose: () => void }) {
  ...
  return (
    <div className="fixed inset-0 z-[9998] grid place-items-center bg-black/40 backdrop-blur-sm p-4" onClick={onClose}>
      <div
        className="relative w-full max-w-6xl rounded-[28px] border border-violet-200/60 bg-white shadow-[0_30px_90px_rgba(99,102,241,0.35)]"
        onClick={(e) => e.stopPropagation()}
      >
        {/* Left rail */}
        <div className="absolute left-0 top-0 h-full w-16 rounded-l-[28px] bg-gradient-to-b from-indigo-600 via-violet-600 to-fuchsia-600 text-white">
        ...
        <div className="pl-16">
          <div className="grid grid-cols-12 gap-6 p-6">
            ...
            <div className="col-span-7">
              <div className="rounded-3xl border border-gray-100 bg-white/90 p-6 shadow-sm">
                ...
                <div className="mt-3 grid gap-3 md:grid-cols-2">
                  {(data?.chapters?.length ? data.chapters : []).slice(0, 2).map((ch) => (
                    <div key={ch.id} className="flex flex-col justify-between rounded-2xl border border-gray-100 p-4 shadow-sm">
                      ...
                      <div className="mt-3 flex items-center gap-2">
                        <a href={`/course/${encodeURIComponent(ch.course_slug)}`} className="rounded-full bg-indigo-600 px-3 py-1.5 text-sm font-semibold text-white hover:bg-indigo-700">Continue</a>
                  ...
                  {!loading && (data?.chapters?.length ?? 0) === 0 && (
                    <div className="col-span-2 rounded-2xl border border-dashed p-6 text-sm text-gray-600">
              ...
              {/* Learning activity */}
              <div className="mt-6 rounded-3xl border border-gray-100 bg-white/90 p-6 shadow-sm">
              ...
              <div className="rounded-3xl border border-gray-100 bg-white/90 p-5 shadow-sm">
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <div className="text-2xl font-extrabold text-indigo-700">{data?.user?.xp ?? 0} XP</div>
                  </div>
                  <div className="flex gap-2">
                    ...
                    <button className="rounded-full bg-emerald-500 px-3 py-1.5 text-sm font-semibold text-white hover:bg-emerald-600">Collect points</button>
                  </div>
                </div>
              </div>
            </div>
            ...
            <div className="col-span-12">
              <div className="rounded-3xl border border-gray-100 bg-white/90 p-6 shadow-sm">
                ...
                <div className="mt-3 grid gap-3 sm:grid-cols-2 md:grid-cols-3">
                  {(data?.courses || []).map((c) => (
                    <a key={c.id} href={`/course/${encodeURIComponent(c.slug)}`} className="flex items-center gap-3 rounded-2xl border p-4 hover:bg-indigo-50/40">
                      <span className="grid h-10 w-10 place-items-center rounded-xl bg-gradient-to-br from-indigo-200 to-fuchsia-200" />
                      <div>
