...
export default function SubtitlesPanel({ tracks, className }: Props) {
  ...
  const lines = useMemo(() => {
    ...
    return parseSrt(pick?.srt || demoSrt);
  }, [tracks, activeLang]);
    ...
    <div className={`rounded-2xl border bg-white/90 p-3 shadow-sm ring-1 ring-black/5 ${className || ""}`}>
      <div className="flex items-center justify-between gap-3">
        ...
        <div className="flex items-center gap-2">
          <button onClick={() => setPlaying((p) => !p)} className="rounded-full bg-indigo-600 px-3 py-1 text-xs font-semibold text-white hover:bg-indigo-700">
      ...
      <div className="relative mt-2 h-24 overflow-hidden">
        ...
        <div className="flex h-full flex-col items-center justify-center gap-1 transition-transform duration-500" style={{ transform: `translateY(-${8}px)` }}>
          <div className="text-center text-[12px] text-gray-400 line-clamp-2">
            {prev?.text || ""}
          </div>
          <div className="rounded-md bg-indigo-50/60 px-3 py-1 text-center text-[14px] font-semibold text-indigo-900 shadow-sm">
            {curr?.text || "Add an .srt file to enable subtitles"}
          </div>
          <div className="text-center text-[12px] text-gray-400 line-clamp-2">
            {next?.text || ""}
          </div>
