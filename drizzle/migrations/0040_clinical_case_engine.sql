...
JOIN (
  VALUES
    ...
    ('scene-initial-action', 'start-therapy', 'Start ketoconazole immediately', 'Treating before confirming etiology hides the signal.', 'Temporary relief but delays localization.', false, 'scene-labs', 15, -4, '[]'::jsonb, '{"feedback":"Therapy before diagnosis risks masking key data."}'::jsonb),
    ('scene-initial-action', 'early-mri', 'Order pituitary MRI now', 'Imaging before biochemical proof can mislead.', 'Incidentalomas may confuse localization.', false, 'scene-labs', 25, -2, '[]'::jsonb, '{"feedback":"Image after biochemical confirmation to avoid false leads."}'::jsonb),
    ...
    ('scene-labs', 'inferior-petrosal', 'Schedule inferior petrosal sinus sampling', 'Invasive and premature with positive MRI pending.', 'High resource cost when not yet indicated.', false, 'scene-diagnosis', 90, -5, '[]'::jsonb, '{"feedback":"Reserve sinus sampling for discordant labs."}'::jsonb),
    ('scene-labs', 'repeat-baseline', 'Repeat the low-dose dex test', 'Redundant testing wastes time.', 'No additional insight gained.', false, 'scene-diagnosis', 20, -3, '[]'::jsonb, '{"feedback":"Redundant testing adds fatigue and no clarity."}'::jsonb),
    ...
    ('scene-diagnosis', 'ectopic-acth', 'Ectopic ACTH production', 'Would not suppress with high-dose dex.', 'Consider only if labs fail to suppress.', false, 'scene-management-intro', 15, -6, '[]'::jsonb, '{"feedback":"Lab pattern argues against ectopic source."}'::jsonb),
    ('scene-diagnosis', 'adrenal-adenoma', 'Adrenal cortisol adenoma', 'Typically ACTH suppressed in adrenal disease.', 'Data does not support this path.', false, 'scene-management-intro', 15, -5, '[]'::jsonb, '{"feedback":"ACTH-dependent labs make adrenal source unlikely."}'::jsonb),
    ...
    ('scene-management', 'ketoconazole-bridge', 'Bridge with ketoconazole', 'Useful temporizing step but not definitive.', 'Best reserved when surgery delayed.', false, 'scene-summary', 20, -4, '[]'::jsonb, '{"feedback":"Medical therapy is a bridge, not the finish line."}'::jsonb),
    ('scene-management', 'adrenalectomy', 'Schedule bilateral adrenalectomy', 'High morbidity salvage for refractory disease.', 'Not first-line for surgical candidates.', false, 'scene-summary', 45, -6, '[]'::jsonb, '{"feedback":"Reserve adrenalectomy for refractory cases."}'::jsonb)
